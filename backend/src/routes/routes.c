...
void handle_control_message(int socket, ControlMessage *msg)
{
    ...
    else if (strcmp(msg->type, SUBMIT_EXAM) == 0)
    {
        handle_submit_exam(socket, msg);
    }
    else if (strcmp(msg->type, USER_START_EXAM) == 0)
    {
    ...
    else if (strcmp(msg->type, GET_PROFILE_BY_ID) == 0)
    {
    ...
    else if (strcmp(msg->type, END_EXAM) == 0)
    {
        handle_end_exam(socket, msg);
    }
}
